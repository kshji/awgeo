...
# $AWMML/get.mml.kiinteistokartta.sh -u 0 -o mml N5424L
#
PRG="$0"
BINDIR="${PRG%/*}"
	...
	do
		...
		case "$tablename" in
				...
				ogrinfo $quit "$Zinf" -sql "DROP TABLE $tablename"
                                ;;
                        t|palstatunnus) # TEXT
				[ -f "$Ztilename.$tablename.csv" ] && $AWGEO/csv2dxf.sh --type t --csv "$Ztilename.$tablename.csv" -d $DEBUG  > "$Ztilename.$tablename.dxf"
				# drop this table from gpkg
				dbg "      " ogrinfo "$Zinf" -sql "DROP TABLE $tablename"
				ogrinfo $quit "$Zinf" -sql "DROP TABLE $tablename"
                                ;;
...
do
	...
	case "$arg" in
		...
		-n|--tilename) tilename=$2; shift ;;
		-m|--makeocad) makeocad=$2; shift ;;
		-*) usage; exit 4 ;;
		*) break ;;
	esac
	shift
...
do
	...
	# if exist old, rm it
	((zipcnt<2)) && rm -f "$outdir"/"$tilename".gpkg "$outdir"/"$tilename".full.gpkg 2>/dev/null

	dbg shp2gpkg "zipf:$zipf" "outdir:$outdir" "sourcedir:$dir" "zipfile:$zipfile" "tilename:$tilename"
	shp2gpkg "$zipf" "$outdir" "$dir" "$zipfile" "$tilename"
done
