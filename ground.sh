...
# https://awot.fi
#
# Make spike free digital surface model - ground
# hillshade "ground" 
#
VER="2024-11-17.a"
#
...
#
#
# ground.sh -i lazfile -o outputdir -s step -d $DEBUG
# ground.sh -i P5331A4.laz 
#  - result file is ./P5331A4.forest.png
# ground.sh -i P5331A4.laz -o results
#  - output to the dir results
#
# ground.sh -i P5331A4.laz -o results -s 0.25 -d 1
...
if [ -f "$AWGEO/config/rgb.rainbow.txt" ] ; then
	...
	dbg las2dem64 $DEMOMODE -i $TEMP.normalized.laz -last_only -step 0.5 -o $TEMP.fo.tif
	las2dem64 $DEMOMODE -i $TEMP.normalized.laz  -last_only -step 0.5 -o "$outputdir/$name.ground.png"
        cp -f "$outputdir/$name.ground.wld" "$outputdir/$name.ground.pgw"
